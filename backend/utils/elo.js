 ...
 * Handles: Fair Play, Cheater Penalties, and Effort Bonuses.
 */
...
export const calculateMatchOutcome = (p1, p2) => {
    ...
    // Formula: E = 1 / (1 + 10^((R_opponent - R_self) / 400))
    ...
    if (p1.isCheater && p2.isCheater) {
        return {
            p1: { newRating: Math.max(0, p1Rating - CHEATER_PENALTY), pointsGained: -CHEATER_PENALTY, seasonScore: 0 },
            p2: { newRating: Math.max(0, p2Rating - CHEATER_PENALTY), pointsGained: -CHEATER_PENALTY, seasonScore: 0 }
        };
    }
    ...
    if (p1.isCheater || p2.isCheater) {
        ...
        const cheater = p1.isCheater ? p1 : p2;
        ...
        const results = {
            p1: p1.isCheater 
                ? { newRating: Math.max(0, p1Rating - CHEATER_PENALTY), pointsGained: -CHEATER_PENALTY, seasonScore: 0 }
                : { newRating: p1Rating + fairEloGain, pointsGained: fairEloGain, seasonScore: 20 + p1.score },
            p2: p2.isCheater 
                ? { newRating: Math.max(0, p2Rating - CHEATER_PENALTY), pointsGained: -CHEATER_PENALTY, seasonScore: 0 }
                : { newRating: p2Rating + fairEloGain, pointsGained: fairEloGain, seasonScore: 20 + p2.score }
        };
        return results;
    }
    ...
    if (p1.score > p2.score) p1Actual = 1;
    else if (p2.score > p1.score) p1Actual = 0;
    ...
    // EFFORT BONUS: Award Elo based on problems solved (score)
    // This allows a loser to still "gain" or "break even" if they performed well
