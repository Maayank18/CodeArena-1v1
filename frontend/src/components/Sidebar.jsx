...
// UPDATED RESPONSIVE CHANGE 
import React, { useEffect, useState } from 'react';
import { Swords, History, Trophy, BookOpen, Globe, LayoutDashboard } from 'lucide-react';
import { useLocation, useNavigate } from 'react-router-dom';
import { io } from 'socket.io-client';
...
const Sidebar = () => {
  ...
  const menu = [
    ...
    { name: 'Ranks', icon: Trophy, path: '/leaderboard' }, // Shortened for mobile
  ...
  return (
    <>
      ...
      {/* -------------------------------------------------------- */}
      <aside className="hidden md:flex w-64 border-r border-[var(--border-color)] bg-[var(--bg-secondary)] flex-col py-6 transition-colors duration-300 h-full">
        <div className="px-4 mb-6">
          <h3 className="text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider px-2">Main Menu</h3>
        </div>
        ...
        <div className="flex flex-col gap-1 px-3">
          {menu.map((item) => {
            ...
            return (
              ...
              >
                <item.icon size={18} />
                {item.name === 'Ranks' ? 'Leaderboard' : item.name === 'Learn' ? 'Resources' : item.name === 'Battle' ? 'Battle Arena' : item.name}
              </button>
            );
          })}
        </div>
      ...
      {/* -------------------------------------------------------- */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-[var(--bg-secondary)] border-t border-[var(--border-color)] flex justify-around items-center px-2 z-50 safe-area-pb">
         {menu.map((item) => {
            const isActive = location.pathname === item.path;
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center w-full h-full gap-1 ${
                  isActive ? 'text-accent' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'
                }`}
              >
                <div className={`p-1.5 rounded-full transition-all ${isActive ? 'bg-accent/10' : ''}`}>
