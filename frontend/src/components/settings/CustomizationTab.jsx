...
import { ADVANCED_THEME_DEFINITIONS, getAdvancedThemeMeta } from '../../utils/advancedThemes';
...
const CustomizationTab = () => {
  ...
  const handleThemeClick = (theme) => {
    ...
    setAdvancedTheme(theme.id);
    const toastConfig = ADVANCED_THEME_TOASTS[theme.id];
    if (toastConfig) {
      toast.success(toastConfig.message, { style: toastConfig.style });
    }
  };
  ...
  useEffect(() => {
    const fetchCustomization = async () => {
      try {
        ...
        if (res.data?.success) {
          ...
          setAvatarFrame(customization?.avatarFrame || 'none');
          setTagline(customization?.tagline || 'Novice');
          setSignatureStack(customization?.signatureStack || []);
          setEntranceBanner(customization?.entranceBanner || 'default-dark');
          if (customization?.advancedTheme) {
            setAdvancedTheme(customization.advancedTheme);
          } else {
            clearAdvancedTheme();
          }
        }
    ...
    fetchCustomization();
  }, [clearAdvancedTheme, setAdvancedTheme]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        const currentStoredUser = JSON.parse(localStorage.getItem('codearena_user') || '{}');
        const nextUser = res.data.user || { ...currentStoredUser, customization: res.data.customization };
  ...
  return (
    <div className="space-y-10">
      ...
      <section>
        ...
        <div className="max-w-md">
          <input
            type="text"
            value={tagline}
            onChange={(event) => setTagline(event.target.value.slice(0, 30))}
      ...
      <section>
        <div className="flex items-center gap-2 mb-4">
          <Palette size={18} className={activeAdvancedTheme?.id === 'frostbyte' ? 'text-cyan-400' : 'text-rose-400'} />
          ...
          <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-tighter ${
            ...
            {activeAdvancedTheme ? `${activeAdvancedTheme.name} Active` : 'Premium Exclusive'}
          </span>
        </div>
