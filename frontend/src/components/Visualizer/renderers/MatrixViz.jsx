import React, { useMemo, memo } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { useThemeColors } from './useThemeColors';

const MatrixViz = memo(({ data, pointers }) => {
    const colors = useThemeColors();
    const isLight = colors.bgPrimary === '#fafaf9';

    if (!data || !Array.isArray(data) || data.length === 0) {
        return (
            <div
                className="p-8 border-2 border-dashed rounded-xl flex items-center justify-center opacity-70"
                style={{
                    borderColor: colors.borderStrong,
                    background: isLight ? 'rgba(255,255,255,0.88)' : 'rgba(13,17,23,0.5)',
                }}
            >
                <span className="font-mono text-xs" style={{ color: colors.textMuted }}>Empty Matrix</span>
            </div>
        );
    }

    const rows = data.length;
    const cols = data[0]?.length || 0;

    const { cellSize, fontSize, gapSize } = useMemo(() => {
        const totalCells = rows * cols;

        if (totalCells <= 25) return { cellSize: 48, fontSize: 'text-sm', gapSize: 8 };
        if (totalCells <= 100) return { cellSize: 36, fontSize: 'text-xs', gapSize: 6 };
        if (totalCells <= 400) return { cellSize: 28, fontSize: 'text-[10px]', gapSize: 4 };
        return { cellSize: 20, fontSize: 'text-[8px]', gapSize: 2 };
    }, [rows, cols]);

    const extractedPointers = useMemo(() => {
        if (!pointers) {
            return {
                current: { row: -1, col: -1 },
                start: { row: -1, col: -1 },
                end: { row: -1, col: -1 },
            };
        }

        return {
            current: {
                row: pointers.i ?? pointers.row ?? pointers.r ?? pointers.x ?? -1,
                col: pointers.j ?? pointers.col ?? pointers.c ?? pointers.y ?? -1,
            },
            start: {
                row: pointers.startRow ?? pointers.start_row ?? pointers.sr ?? -1,
                col: pointers.startCol ?? pointers.start_col ?? pointers.sc ?? -1,
            },
            end: {
                row: pointers.endRow ?? pointers.end_row ?? pointers.er ?? -1,
                col: pointers.endCol ?? pointers.end_col ?? pointers.ec ?? -1,
            },
        };
    }, [pointers]);

    return (
        <div
            className="inline-block p-6 border rounded-[28px] shadow-2xl overflow-auto custom-scrollbar"
            style={{
                maxHeight: '600px',
                maxWidth: '90vw',
                borderColor: colors.border,
                background: `linear-gradient(180deg, ${colors.bgSecondary} 0%, ${colors.bgPrimary} 100%)`,
                boxShadow: isLight
                    ? '0 20px 46px rgba(15, 23, 42, 0.08), inset 0 1px 0 rgba(255,255,255,0.85)'
                    : '0 18px 40px rgba(0,0,0,0.28)',
            }}
        >
            <div className="flex flex-col relative">
                <div className="flex items-center justify-between mb-3 pb-2 border-b" style={{ borderColor: colors.border }}>
                    <span className="text-[10px] font-mono" style={{ color: colors.textMuted }}>
                        Dimensions: {rows}x{cols}
                    </span>
                    {extractedPointers.current.row >= 0 && extractedPointers.current.col >= 0 && (
                        <span className="text-[10px] font-mono font-bold" style={{ color: colors.activeText }}>
                            [{extractedPointers.current.row}][{extractedPointers.current.col}]
                        </span>
                    )}
                </div>

                <div className="flex mb-2 ml-8">
                    <div style={{ display: 'flex', gap: `${gapSize}px` }}>
                        {Array.from({ length: cols }).map((_, c) => (
                            <div
                                key={c}
                                className="text-center text-[9px] font-mono select-none font-bold"
                                style={{ width: cellSize, color: colors.textFaint }}
                            >
                                {c}
                            </div>
                        ))}
                    </div>
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: `${gapSize}px` }}>
                    {data.map((row, r) => (
                        <div key={r} className="flex items-center" style={{ gap: `${gapSize}px` }}>
                            <div className="w-6 text-right text-[9px] font-mono select-none pr-2 font-bold" style={{ color: colors.textFaint }}>
                                {r}
                            </div>
                            <MatrixRow
                                row={row}
                                rowIndex={r}
                                extractedPointers={extractedPointers}
                                cellSize={cellSize}
                                fontSize={fontSize}
                                gapSize={gapSize}
                                colors={colors}
                                isLight={isLight}
                            />
                        </div>
                    ))}
                </div>
            </div>
        </div>
    );
});

const MatrixRow = memo(({ row, rowIndex, extractedPointers, cellSize, fontSize, gapSize, colors, isLight }) => (
    <div style={{ display: 'flex', gap: `${gapSize}px` }}>
        {row.map((val, colIndex) => {
            const r = rowIndex;
            const c = colIndex;
            const { current, start, end } = extractedPointers;

            const isCurrentCell = current.row === r && current.col === c;
            const isRowActive = current.row === r && current.col === -1;
            const isStartCell = start.row === r && start.col === c;
            const isEndCell = end.row === r && end.col === c;
            const isInRange =
                start.row >= 0 && end.row >= 0 &&
                r >= start.row && r <= end.row &&
                c >= start.col && c <= end.col;

            return (
                <MatrixCell
                    key={`${r}-${c}`}
                    val={val}
                    r={r}
                    c={c}
                    isCurrentCell={isCurrentCell}
                    isRowActive={isRowActive}
                    isStartCell={isStartCell}
                    isEndCell={isEndCell}
                    isInRange={isInRange}
                    cellMeta={detectCellMetadata(val)}
                    cellSize={cellSize}
                    fontSize={fontSize}
                    colors={colors}
                    isLight={isLight}
                />
            );
        })}
    </div>
));

function detectCellMetadata(val) {
    if (val && typeof val === 'object') {
        return {
            isObject: true,
            isVisited: val.visited || val.isVisited || false,
            isPath: val.path || val.isPath || false,
            isObstacle: val.obstacle || val.isObstacle || val.blocked || false,
            isStart: val.start || val.isStart || false,
            isEnd: val.end || val.isEnd || val.goal || false,
            distance: val.distance || val.dist || val.cost || null,
            value: val.val || val.value || val.data || '?',
        };
    }

    return {
        isObject: false,
        isVisited: false,
        isPath: false,
        isObstacle: val === '#' || val === 'X' || val === -1 || val === Infinity,
        isStart: val === 'S',
        isEnd: val === 'E' || val === 'G',
        distance: null,
        value: val,
    };
}

const MatrixCell = memo(({
    val,
    r,
    c,
    isCurrentCell,
    isRowActive,
    isStartCell,
    isEndCell,
    isInRange,
    cellMeta,
    cellSize,
    fontSize,
    colors,
    isLight,
}) => {
    const getCellStyle = () => {
        if (isCurrentCell) {
            return {
                bg: colors.activeCell,
                border: colors.activeBorder,
                text: colors.activeText,
                shadow: isLight
                    ? '0 14px 30px rgba(245, 158, 11, 0.16), 0 0 0 4px rgba(245, 158, 11, 0.08)'
                    : '0 0 15px rgba(245, 158, 11, 0.3)',
                scale: 1.1,
                zIndex: 30,
            };
        }

        if (cellMeta.isStart || isStartCell) {
            return {
                bg: 'rgba(34, 197, 94, 0.18)',
                border: '#22c55e',
                text: isLight ? '#166534' : '#86efac',
                shadow: '0 0 10px rgba(34, 197, 94, 0.2)',
                scale: 1.05,
                zIndex: 25,
            };
        }

        if (cellMeta.isEnd || isEndCell) {
            return {
                bg: 'rgba(239, 68, 68, 0.16)',
                border: '#ef4444',
                text: isLight ? '#991b1b' : '#fca5a5',
                shadow: '0 0 10px rgba(239, 68, 68, 0.2)',
                scale: 1.05,
                zIndex: 25,
            };
        }

        if (cellMeta.isObstacle) {
            return {
                bg: isLight ? '#e2e8f0' : colors.bgHover,
                border: isLight ? '#cbd5e1' : colors.borderStrong,
                text: colors.textMuted,
                shadow: 'none',
                scale: 1,
                zIndex: 5,
            };
        }

        if (cellMeta.isPath) {
            return {
                bg: 'rgba(59, 130, 246, 0.14)',
                border: '#3b82f6',
                text: isLight ? '#1d4ed8' : '#93c5fd',
                shadow: '0 0 8px rgba(59, 130, 246, 0.15)',
                scale: 1,
                zIndex: 15,
            };
        }

        if (cellMeta.isVisited) {
            return {
                bg: 'rgba(168, 85, 247, 0.10)',
                border: '#a855f7',
                text: isLight ? '#7e22ce' : '#d8b4fe',
                shadow: 'none',
                scale: 1,
                zIndex: 10,
            };
        }

        if (isRowActive) {
            return {
                bg: colors.bgHover,
                border: colors.borderStrong,
                text: colors.textSecondary,
                shadow: 'none',
                scale: 1,
                zIndex: 8,
            };
        }

        if (isInRange) {
            return {
                bg: 'rgba(100, 116, 139, 0.10)',
                border: '#64748b',
                text: colors.textSecondary,
                shadow: 'none',
                scale: 1,
                zIndex: 6,
            };
        }

        return {
            bg: colors.bgCard,
            border: colors.borderStrong,
            text: colors.textPrimary,
            shadow: isLight ? '0 10px 24px rgba(15, 23, 42, 0.06)' : 'none',
            scale: 1,
            zIndex: 0,
        };
    };

    const style = getCellStyle();
    const displayValue = cellMeta.isObject ? cellMeta.value : val;

    return (
        <motion.div
            layout
            initial={false}
            animate={{
                scale: style.scale,
                backgroundColor: style.bg,
                borderColor: style.border,
                boxShadow: style.shadow,
                zIndex: style.zIndex,
            }}
            transition={{ duration: 0.2, type: 'spring', stiffness: 300, damping: 25 }}
            className={`flex items-center justify-center border-2 rounded-lg relative ${fontSize} font-mono font-bold`}
            style={{ width: cellSize, height: cellSize, color: style.text }}
        >
            <span className="z-10 truncate px-0.5">
                {displayValue === null ? 'null'
                    : displayValue === undefined ? '?'
                    : displayValue === Infinity ? 'INF'
                    : displayValue === -Infinity ? '-INF'
                    : String(displayValue)}
            </span>

            {!isCurrentCell && cellSize >= 36 && (
                <span className="absolute bottom-0.5 left-0.5 text-[7px] opacity-40" style={{ color: colors.textFaint }}>
                    {r},{c}
                </span>
            )}

            {cellMeta.distance !== null && cellSize >= 36 && (
                <span
                    className="absolute top-0.5 right-0.5 text-[7px] px-1 rounded font-bold"
                    style={{
                        background: isLight ? 'rgba(255,255,255,0.9)' : 'rgba(13,17,23,0.8)',
                        color: colors.textMuted,
                        border: `1px solid ${colors.border}`,
                    }}
                >
                    {cellMeta.distance}
                </span>
            )}

            <AnimatePresence>
                {isCurrentCell && cellSize >= 28 && (
                    <motion.div
                        initial={{ opacity: 0, y: 5 }}
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0 }}
                        className="absolute -top-3 -right-1 z-40"
                    >
                        <div
                            className="text-[8px] px-1.5 py-0.5 rounded shadow-lg font-bold border whitespace-nowrap"
                            style={{ background: '#f59e0b', color: colors.bgPrimary, borderColor: '#d97706' }}
                        >
                            [{r}][{c}]
                        </div>
                    </motion.div>
                )}
            </AnimatePresence>

            {(cellMeta.isStart || isStartCell) && cellSize >= 28 && (
                <div className="absolute -top-2 left-1/2 -translate-x-1/2 text-[8px] bg-green-500 text-black px-1 py-0.5 rounded font-bold border border-green-600">
                    S
                </div>
            )}

            {(cellMeta.isEnd || isEndCell) && cellSize >= 28 && (
                <div className="absolute -top-2 left-1/2 -translate-x-1/2 text-[8px] bg-red-500 text-black px-1 py-0.5 rounded font-bold border border-red-600">
                    E
                </div>
            )}
        </motion.div>
    );
});

export default MatrixViz;
