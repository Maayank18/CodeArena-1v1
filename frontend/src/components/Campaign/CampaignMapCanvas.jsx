...
// ─────────────────────────────────────────────────────────────────────────────
// ALL FIXES APPLIED:
//   • bgGrad/titleGrad: must use,, — they are arrays, NOT strings
//   • Stars: 3-star system (was showing 5)
//   • node.problem vs node.problemId: both field names supported
//   • zone.nodes safe fallback: (zone.nodes || [])
//   • Legend: absolute bottom-6 left-6 z-50 pointer-events-none — never blocks
...
//   • Theme: dark: semantic classes throughout
//   • Mobile: legend/hint shrink on xs screens
//   • CRITICAL CRASH FIX: Added strict null-safety to segPath and loops 
//     to prevent "TypeError: a is undefined" when data is syncing.
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Bezier path between two nodes ─────────────────────────────────────────────
const segPath = (a, b) => {
  // 🛡️ CRITICAL FIX: Prevent crash if node 'a' or 'b' is undefined during map sync
...
// ── Standard node ─────────────────────────────────────────────────────────────
const StdNode = ({ node, accent, onClick }) => {
  if (!node) return null; // Safe fallback
...
const CampaignMapCanvas = ({ zone, onBack, onNodeClick }) => {
  ...
  const segments = useMemo(
    () =>
      nodePositions.slice(0, -1).map((a, i) => {
        const b = nodePositions[i + 1];
        // 🛡️ CRITICAL FIX: Ensure both nodes exist before attempting path drawing
        if (!a || !b) return null; 
    ...
    >
      ...
      >
        ...
        {nodePositions.map((node, i) => {
          // 🛡️ CRITICAL FIX: Ensure the node and its coordinates exist before mounting
