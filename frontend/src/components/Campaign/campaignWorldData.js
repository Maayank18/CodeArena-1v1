...
export const getZoneConfig = (zoneId) => ZONE_CONFIGS.find(z => z.id === zoneId) || ZONE_CONFIGS;

// ─────────────────────────────────────────────────────────────────────────────
// BULLETPROOF DATA STITCHING
// Wildcard imports (*) ensure we don't crash regardless of whether the data 
// files use `export default` or `export const`.
// ─────────────────────────────────────────────────────────────────────────────

import * as batch1 from '../../data/campaignData';
import * as batch2 from '../../data/campaignDataBatch2';

// Extract the arrays safely. Fallback to empty array if missing.
...
export const generateMockWorld = () => {
  ...
  ALL_ZONES.forEach((zone, zIdx) => {
    // Failsafe in case zone is malformed
    ...
    zone.nodes.forEach((node, nIdx) => {
      nodes.push({
        ...
        // Failsafe prerequisite logic so it doesn't crash reading ALL_ZONES[zIdx-1]
        prerequisites: nIdx === 0
          ? (zIdx === 0 || !ALL_ZONES[zIdx-1] ? [] : [`${ALL_ZONES[zIdx-1].id}_15`])
          : [`${zone.id}_${nIdx}`],
