...
//     useEffect(() => {
//     const init = async () => {
//         // 1️⃣ Initialize Yjs provider ONCE
//         if (!providerRef.current) {
//             console.log('🟡 Initializing Yjs provider for room:', roomId);
...
//             // 3️⃣ Register room_joined listener ONCE
//             socketRef.current.on('room_joined', (data) => {
//                 console.log('🟢 room_joined - MY side:', data.side, 'My username:', location.state?.username);
...
//             // 4️⃣ Register player_joined listener ONCE
...
//             // 5️⃣ Register other listeners ONCE
//             socketRef.current.on('new_round', (data) => {
//                 toast.success(`Round ${data.round} Started!`);
//                 setProblem(data.problem);
//                 setRound(data.round);
...
//             // 6️⃣ Now emit join_room AFTER all listeners are registered
//             console.log('📤 Emitting join_room for:', location.state?.username);
...
//     return () => {
//         console.log('🧹 Cleanup: removing listeners and disconnecting');
//         if(socketRef.current) {
...
//     //         // ✅ Handle YOUR join - this sets YOUR side ONCE
//     //         socketRef.current.on('room_joined', (data) => {
//     //             console.log('🟢 room_joined - MY side:', data.side, 'My username:', location.state?.username);
//     //             setClients(data.players);
//     //             setMySide(data.side);  // ✅ Set YOUR side here
//     //             setProblem(data.problem);
//     //             setRound(data.round);
//     //             setTotalRounds(data.totalRounds);
//     //             setScores(data.scores);
...
//     //         // ✅ Handle when OTHER player joins - NEVER touch mySide
//     //         socketRef.current.on('player_joined', ({ username, side, players, scores }) => {
//     //             console.log('🔵 player_joined - other player:', username, 'their side:', side, 'My side stays:', mySide);
//     //             setClients(players);  // Update player list
//     //             setScores(scores);    // Update scores
//     //             // ❌ NEVER EVER set mySide here!
//     //             toast.success(`${username} joined the room!`);
//     //         });
...
//   const ydocRef = useRef(new Y.Doc());
//   const providerRef = useRef(null);
//   const [providerReady, setProviderReady] = useState(false); // only render editors once provider ready

//   // Initialize provider + socket once
//   useEffect(() => {
//     let mounted = true;

//     const init = async () => {
//       try {
//         // Create provider (synchronously here so CodeEditor won't mount before it exists)
//         if (!providerRef.current) {
//           console.log('🟡 Initializing Yjs provider for room:', roomId);
//           providerRef.current = new WebsocketProvider(import.meta.env.VITE_YJS_URL, roomId, ydocRef.current);
//           providerRef.current.on('status', (ev) => {
//             console.log('Yjs provider status:', ev.status);
//             if (mounted && ev.status === 'connected') setProviderReady(true);
//           });
//           // If provider is already connected by the time we set listener, mark ready.
//           if (providerRef.current.wsconnected || providerRef.current.connected) {
...
//           // room_joined: only for the socket that joined
//           socketRef.current.on('room_joined', (data) => {
//             console.log('🟢 room_joined - MY side:', data.side, 'My username:', location.state?.username);
//             setClients(data.players || []);
//             setMySide(data.side);
...
//             // Set awareness (if provider already exists)
//             if (providerRef.current?.awareness) {
//               providerRef.current.awareness.setLocalStateField('user', {
//                 name: location.state?.username,
//                 color: data.side === 'left' ? '#007acc' : '#ff0000',
...
//           // Finally join room AFTER listeners registered
//           console.log('📤 Emitting join_room for:', location.state?.username);
//           socketRef.current.emit('join_room', { roomId, username: location.state?.username });
//         }
//       } catch (err) {
...
//     // Attempt to read username from location.state or localStorage
//     if (location.state?.username) {
//       init();
//     } else {
//       const storedUser = JSON.parse(localStorage.getItem('codearena_user') || 'null');
//       if (storedUser && storedUser.username) {
//         // set on state so the init uses it
//         // mutate location.state only as a fallback to keep rest of code working
//         location.state = { username: storedUser.username };
//         init();
//       } else {
//         navigate('/login');
...
//       }
//     };
//     // NOTE: this effect intentionally runs once on mount
//   }, []); // run ONCE
...
//   const runCode = async () => {
//     if (!mySide) {
//       toast.error('Side not set yet. Wait for connection.');
//       return;
//     }
//     if (!providerRef.current) {
//       toast.error('Collaboration provider not ready. Try again shortly.');
//       return;
//     }

//     setIsRunning(true);
//     const sharedText = ydocRef.current.getText(`code-${mySide}`);
//     const code = sharedText ? sharedText.toString() : '';
//     if (!code) {
...
//       for (const [index, tc] of publicCases.entries()) {
//         try {
//           const response = await api.post('/api/run', { language: language, code: code, stdin: tc.input });
//           const actualOutput = response.data.stdout ? response.data.stdout.trim() : '';
//           const expectedOutput = (tc.output || '').trim();
//           const passed = actualOutput === expectedOutput;
//           newResults.push({
...
//           });
//         } catch (err) {
//           newResults.push({ type: 'error', id: index, input: tc.input, error: 'API Error', passed: false });
//         }
//       }
//       setRunResults(newResults);
//       toast.success('Run Complete');
...
//   // if no auth info redirect
//   if (!location.state && !localStorage.getItem('codearena_user')) return <Navigate to="/" />;

//   // show spinner while waiting for essential info
//   if ((!mySide || !problem) && location.state?.username) {
//     return (
//       <div className="h-screen bg-[#121212] text-white flex flex-col items-center justify-center gap-4">
//         <div className="animate-spin rounded-full h-12 w-12 border-t-4 border-green-500 border-r-4 border-green-500/30"></div>
...
//   // safe score name lookups (avoid using "Waiting..." as a scores key)
//   const leftName = getPlayerName('left');
//   const rightName = getPlayerName('right');
...
//           <div className="bg-[#2d2d2d] p-3 flex justify-between items-center border-b border-[#3e3e42] shrink-0 h-14">
//             <div className="flex items-center gap-2 overflow-hidden">
//               <span className="font-bold text-sm truncate text-white max-w-[120px]">{leftName}</span>
//               <span className="bg-black/50 px-2 py-0.5 rounded text-green-400 text-xs font-mono shrink-0">{scores[leftName] || 0} pts</span>
//               {mySide === 'left' && <span className="text-accent text-[10px] font-bold bg-accent/10 px-1 rounded border border-accent/50">YOU</span>}
//             </div>
//             {mySide === 'left' && (
//               <select className="bg-[#3e3e42] text-xs text-white p-1 rounded border border-[#555] outline-none" value={language} onChange={(e) => setLanguage(e.target.value)}>
...
//           <div className="flex-1 relative min-h-0">
//             {/* Render editors only once provider is ready to avoid blank/editor crashes */}
//             {providerReady ? (
//               <CodeEditor
//                 key={`${roomId}-left-${providerReady}-${mySide}`}
//                 roomId={roomId}
//                 side="left"
...
//               />
//             ) : (
//               <div className="h-full flex items-center justify-center text-gray-500">Initializing editor...</div>
//             )}
//           </div>
//         </div>
...
//                   <div className="bg-[#1e1e1e] p-4 rounded-lg border border-[#3e3e42]">
//                     <h3 className="text-accent font-bold mb-2 text-xs uppercase tracking-wider">Constraints</h3>
//                     <ul className="list-disc list-inside text-gray-400 space-y-1">{problem.constraints.map((c, i) => <li key={i} className="font-mono text-xs">{c}</li>)}</ul>
//                   </div>
//                 )}
//                 {(problem.testCases || []).filter((tc) => tc.isPublic).map((tc, i) => (
//                   <div key={i} className="mb-4 bg-[#1e1e1e] p-3 rounded border border-[#3e3e42]">
...
//                     <div className="flex items-center gap-2 mb-2">
//                       {res.passed ? <CheckCircle size={14} className="text-green-500" /> : <XCircle size={14} className="text-red-500" />}
//                       <span className={`text-xs font-bold ${res.passed ? 'text-green-400' : 'text-red-400'}`}>{res.passed ? `Test Case ${idx + 1} Passed` : `Test Case ${idx + 1} Failed`}</span>
//                     </div>
//                   </div>
//                 ))}
//               </div>
...
//                 <span className="text-xs font-mono text-white select-all">{roomId}</span>
//               </div>
//               <button onClick={copyRoomId} className="p-2 hover:bg-[#3e3e42] rounded text-gray-400 hover:text-white transition-colors" title="Copy Room ID"><Copy size={16} /></button>
//             </div>
...
//           <div className="bg-[#2d2d2d] p-3 flex justify-between items-center border-b border-[#3e3e42] shrink-0 h-14">
//             <div className="flex items-center gap-2 overflow-hidden">
//               <span className="font-bold text-sm truncate text-white max-w-[120px]">{rightName}</span>
//               <span className="bg-black/50 px-2 py-0.5 rounded text-green-400 text-xs font-mono shrink-0">{scores[rightName] || 0} pts</span>
//             </div>
//             {mySide === 'right' && (
//               <select className="bg-[#3e3e42] text-xs text-white p-1 rounded border border-[#555] outline-none" value={language} onChange={(e) => setLanguage(e.target.value)}>
//                 <option value="cpp">C++</option>
...
//             {providerReady ? (
//               <CodeEditor
//                 key={`${roomId}-right-${providerReady}-${mySide}`}
//                 roomId={roomId}
//                 side="right"
