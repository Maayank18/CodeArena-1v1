...
// PRODUCTION-OPTIMIZED VERSION - Timer persists on refresh
import React, { useState, useRef, useEffect, useCallback } from 'react';
import toast from 'react-hot-toast';
import Client from '../components/Client';
import CodeEditor from '../components/CodeEditor';
...
// ✅ OPTIMIZED TIMER: Persists on refresh + Server sync
const Timer = React.memo(({ socket, roomId }) => {
    const [timeLeft, setTimeLeft] = useState(null); // ✅ Start as null, wait for server
    const intervalRef = useRef(null);
    const serverTimeRef = useRef(null); // ✅ Store server time for reference

    // ✅ CRITICAL: Initialize timer from server on mount
    useEffect(() => {
        if (!socket) return;

        const handleRoomJoined = (data) => {
            if (data.remainingTime !== undefined && data.remainingTime !== null) {
                console.log(`[TIMER] Initialized from server: ${data.remainingTime}s`);
                setTimeLeft(data.remainingTime);
                serverTimeRef.current = data.remainingTime;
            }
        };
    ...
    // ✅ Client-side countdown (only starts when timeLeft is set)
    useEffect(() => {
        if (timeLeft === null) return; // Wait for server initialization
    ...
    }, [timeLeft !== null]); // Only re-run if timeLeft becomes non-null

    // ✅ Server sync (every 60s from backend)
    useEffect(() => {
        if (!socket) return;
        ...
        const handleSyncTime = (serverTime) => {
            ...
            // Only sync if difference > 2 seconds (prevents jitter)
            if (diff > 2) {
                console.log(`[TIMER] Syncing: ${timeLeft}s → ${serverTime}s (drift: ${diff}s)`);
                setTimeLeft(serverTime);
    ...
    const formatTime = (s) => {
        if (s === null) return '--:--'; // ✅ Show placeholder while loading
        const mins = Math.floor(s / 60);
        const secs = s % 60;
        return `${mins}:${secs < 10 ? '0' : ''}${secs}`;
    };
...
const EditorPage = () => {
    ...
    // ✅ OPTIMIZED: UI State with proper memoization
    const [clients, setClients] = useState([]);
    const [problem, setProblem] = useState(null);
    const [mySide, setMySide] = useState(null); 
    const [runResults, setRunResults] = useState(null); 
    ...
    // ✅ OPTIMIZED: Yjs with proper cleanup
    ...
    // ✅ OPTIMIZED: Debounce with ref (prevents memory leaks)
    const debounceTimerRef = useRef(null);

    // ✅ OPTIMIZED: Socket connection with proper cleanup
    useEffect(() => {
    ...
    // ✅ ANTI-CHEAT: Separate effect for cleaner code
    useEffect(() => {
    ...
    // ✅ OPTIMIZED: Code execution with proper error handling
    ...
    return (
        <div className="relative h-screen w-screen bg-dark text-gray-300 overflow-hidden font-sans flex flex-col">
            ...
            {/* ✅ Connection Banner (only shows if previously connected) */}
