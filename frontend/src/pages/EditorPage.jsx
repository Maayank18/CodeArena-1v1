...
const EditorPage = () => {
    ...
    useEffect(() => {
        ...
        const init = async () => {
            ...
            socketRef.current.on('room_joined', (data) => {
                ...
                // Success toast
                toast.success(`Joined as Player ${data.side === 'left' ? 'A' : 'B'}`);
            });
        ...
        return () => {
            if(socketRef.current) socketRef.current.disconnect();
            // Do not destroy provider here to prevent flickering
        };
    }, []);
    ...
    // Loading Spinner - Only show if critical data is missing
    if (!mySide && !problem) {
        return (
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-6 text-sm leading-relaxed min-h-0">
                        {problem ? (
                            <div className="space-y-6 pb-6">
                                ...
                                {problem.constraints && <div><h3 className="text-accent font-bold mb-2 text-xs uppercase tracking-wider">Constraints</h3><ul className="list-disc list-inside text-gray-400 space-y-1">{problem.constraints.map((c, i) => <li key={i} className="font-mono text-xs">{c}</li>)}</ul></div>}
                                <div><h3 className="text-accent font-bold mb-2 text-xs uppercase tracking-wider">Examples</h3>{problem.testCases.filter(tc => tc.isPublic).map((tc, i) => (<div key={i} className="mb-4 bg-[#1e1e1e] p-3 rounded border border-[#3e3e42]"><div className="mb-2"><span className="text-[10px] text-gray-500 font-bold uppercase block mb-1">Input</span><code className="block bg-[#2d2d2d] p-2 rounded text-gray-300 font-mono text-xs break-words whitespace-pre-wrap">{tc.input}</code></div><div><span className="text-[10px] text-gray-500 font-bold uppercase block mb-1">Expected Output</span><code className="block bg-[#2d2d2d] p-2 rounded text-green-400 font-mono text-xs break-words whitespace-pre-wrap">{tc.output}</code></div></div>))}</div>
                            </div>
                        ) : (<div className="flex h-full items-center justify-center text-gray-500 animate-pulse">Loading Problem...</div>)}
                    </div>
