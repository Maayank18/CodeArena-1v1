...
// YE WALA BADIA HAI LEKIN REFRESH WALI PROBLEM SOLVE NHI KAR PAA RAHA ABHI 
...
// // ─── Micro-Components (Isolated to prevent global re-renders) ─────────────────
...
//   const [successResult, setSuccessResult] = useState(null);
//   const [mobileTab,     setMobileTab]     = useState('problem');
//   const [startTime,     setStartTime]     = useState(null); // Used instead of rapid interval state

//   const editorRef = useRef(null);

//   // ── Load node and recover session code ──────────────────────────────────
//   useEffect(() => {
//     let cancelled = false;
//     const load = async () => {
//       setLoading(true);
...
//           // Auto-recovery mechanism for Refresh stability
//           const savedCode = localStorage.getItem(getStorageKey(nodeId, 'javascript'));
...
//     // Only warn if they typed something AND didn't already have it saved in the next lang
//     if (code !== currentStarter && code.trim().length > 0) {
//       if (!window.confirm(`Switch to ${lang}? This will load the starter template for ${lang}.`)) return;
//     }
...
//       } catch (err) {
//         const status  = err.response?.status;
//         const errMsg  = err.response?.data?.message || err.response?.data?.error || err.message || 'Execution service unavailable';
//         const userMsg = status === 503 ? 'Execution service is temporarily unavailable — please retry in a moment' : (status === 500 ? `Server error: ${errMsg}` : errMsg);
...
//       if (data.allPassed) {
//         // Calculate the elapsed time strictly when it passes
//         const elapsedSeconds = Math.floor((Date.now() - startTime) / 1000);
//         setSuccessResult({ ...data, elapsedSeconds });
//         setShowSuccess(true);
//         setFailCount(0);
//         setSageShouldShow(false);
//         // Clean storage on success
...
//     } catch (err) {
//       const status  = err.response?.status;
//       const errMsg  = err.response?.data?.message || err.response?.data?.error || err.message || 'Submission failed';
//       const userMsg = status === 503 ? 'Execution service is temporarily unavailable — please retry in a moment' : (status === 500 ? `Server error: ${errMsg}` : errMsg);
...
//   // Ref-based callback pattern ensures the event listener is attached once
//   // but always executes the freshest version of handleRun/handleSubmit.
...
//         <div className="flex items-center gap-1 text-slate-500 dark:text-gray-600 shrink-0">
//           <Clock size={12} />
//           <TimerDisplay startTime={startTime} isStopped={showSuccess} />
//         </div>
