...
const Campaign = () => {
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-slate-50 dark:bg-[#060810] flex flex-col items-center justify-center gap-5">
                <div className="text-7xl select-none" style={{ animation: 'bounce 1s infinite' }}>🗺️</div>
    ...
    // ── Defensive: normalise mapData shape ──
    const nodes = mapData?.nodes ?? [];

    // ── Empty ────────────────────────────────────────────────────────────────
    if (!nodes.length) {
        return (
            <div className="min-h-screen bg-slate-50 dark:bg-[#060810] flex flex-col">
                <Navbar user={user} onLogout={handleLogout} />
                <div className="flex-1 flex flex-col items-center justify-center gap-5 text-center px-4">
                    <Map size={64} className="text-slate-300 dark:text-gray-800 opacity-40" />
                    <div>
                        <h2 className="text-2xl font-black text-slate-400 dark:text-gray-500 mb-2">No Campaign Nodes Yet</h2>
                        <p className="text-slate-500 dark:text-gray-700 text-sm">Run the seed script to populate the map.</p>
                    ...
                    <button
                        onClick={() => navigate('/dashboard')}
                        className="flex items-center gap-2 px-4 py-2 bg-slate-200 hover:bg-slate-300 dark:bg-gray-800 dark:hover:bg-gray-700 text-slate-700 dark:text-gray-300 rounded-lg transition-colors text-sm font-bold mt-4"
                    >
                        <ArrowLeft size={15} /> Back to Dashboard
                    </button>
                </div>
    ...
    // ── Main ─────────────────────────────────────────────────────────────────
    return (
        <div className="h-screen bg-slate-50 dark:bg-[#060810] flex flex-col overflow-hidden">
            <Navbar user={user} onLogout={handleLogout} />
            ...
            >
                <button
                    onClick={() => setShowGuide(true)}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-slate-500 hover:text-slate-800 hover:bg-slate-200 dark:text-gray-500 dark:hover:text-gray-200 dark:hover:bg-gray-800/60 rounded-lg transition-colors text-xs font-bold border border-transparent dark:hover:border-gray-700/40"
                    title="How to Play"
                >
                    <BookOpen size={14} />
                    <span className="hidden sm:inline">Guide</span>
