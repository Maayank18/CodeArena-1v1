...
const Visualizer = () => {
    ...
    // --- API & EXECUTION ---
    // Moved outside to be accessible by useEffect
    const executeCode = async (codeToRun) => {
        ...
        setTrace([]);
        setCurrentStep(0);
        // setMobileTab('visualizer'); // Don't auto-switch on initial load
        ...
        try {
            ...
            if (data.success && data.trace && data.trace.length > 0) {
                setTrace(data.trace);
                // toast.success(`Success! Generated ${data.trace.length} steps.`); // Optional on auto-run
            } else {
                toast.error('No steps generated. Check your logic.');
            }
        } catch (error) {
    ...
    const handleRun = () => {
        setMobileTab('visualizer'); // Switch tab on manual run
        executeCode(code);
    };
    ...
    const loadExample = (key) => {
        ...
        setMobileTab('editor');
        toast.success(`Loaded ${key} example`);
        // Optional: Auto-run the loaded example?
        // executeCode(newCode); 
    };

    // ✅ AUTO-RUN ON MOUNT
    useEffect(() => {
        // Run the default bubbleSort code immediately when component mounts
