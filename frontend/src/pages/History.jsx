...
// UDPATED HISTORY WITH ANTI CHEAT MECHANISM MESSAGE 
// import React, { useEffect, useState } from 'react';
// import Navbar from '../components/Navbar';
// import Sidebar from '../components/Sidebar';
// import { format } from 'date-fns';
...
//                                     // ***************************************************************
//                                     // ✅ ANTI-CHEAT UI LOGIC START
//                                     // ***************************************************************
//                                     const hasStatusText = myData.statusText && myData.statusText !== "";
//                                     const isDisqualifiedMatch = match.isDisqualified;
//                                     // ***************************************************************

//                                     return (
//                                         <div key={idx} className={`bg-[var(--bg-secondary)] p-4 rounded-xl border flex items-center justify-between hover:border-accent transition-all shadow-sm group ${isDisqualifiedMatch ? 'border-red-500/30' : 'border-[var(--border-color)]'}`}>
                                            
//                                             <div className="flex items-center gap-3 md:gap-6">
//                                                 <div className={`h-10 w-10 md:h-12 md:w-12 rounded-lg flex items-center justify-center font-bold text-lg md:text-xl shrink-0 ${
//                                                     isWin ? 'bg-green-500/10 text-green-500 border border-green-500/20' : 'bg-red-500/10 text-red-500 border border-red-500/20'
//                                                 }`}>
//                                                     {/* ✅ Show Shield icon if match involved cheating */}
//                                                     {isDisqualifiedMatch ? <ShieldAlert size={20} /> : (isWin ? <CheckCircle size={20} /> : <XCircle size={20} />)}
//                                                 </div>
...
//                                             <div className="text-right pl-4 flex flex-col items-end">
//                                                 {/* ✅ DISPLAY CUSTOM STATUS (e.g., Unfair Practice) OR SCORE */}
//                                                 <div className="flex items-baseline gap-1">
//                                                     {hasStatusText ? (
//                                                         <span className={`text-xs md:text-sm font-bold italic ${isWin ? 'text-green-400' : 'text-red-400'}`}>
//                                                             {myData.statusText}
//                                                         </span>
//                                                     ) : (
//                                                         <>
...
// // LITTLE CORECTED AND UPDATED MODIFIED HISTORY BUT STILL WORKING CODE 
// import React, { useEffect, useState } from 'react';
// import Navbar from '../components/Navbar';
// import Sidebar from '../components/Sidebar';
// import { format } from 'date-fns';
...
// MOST OPTIMAL QUALLITY BUT A MORE FIXED VERSION IS BELOW ONE 
// import React, { useEffect, useState } from 'react';
// import Navbar from '../components/Navbar';
// import Sidebar from '../components/Sidebar';
// import { format } from 'date-fns';
// import { History as HistoryIcon, Trophy, Loader2, TrendingUp, TrendingDown, ShieldAlert } from 'lucide-react';
// import axios from 'axios';
// import api from '../api.js';
...
//                                     // ✅ LOGIC: Determine Status Letter (W, L, or D)
//                                     // Checks if the statusText contains "Disqualified" as set by backend
//                                     const isDisqualified = myData.statusText === "Disqualified";
                                    
//                                     let statusLetter = isWin ? "W" : "L";
//                                     let statusColor = isWin 
...
//                                     return (
//                                         <div key={idx} className={`bg-[var(--bg-secondary)] p-4 rounded-xl border flex items-center justify-between hover:border-accent transition-all shadow-sm group ${isDisqualified ? 'border-orange-500/30' : 'border-[var(--border-color)]'}`}>
...
//                                             <div className="text-right pl-4 flex flex-col items-end">
//                                                 <div className="flex items-baseline gap-1">
//                                                     {/* ✅ Show "Disqualified" label or points */}
//                                                     {isDisqualified ? (
//                                                         <span className="text-xs md:text-sm font-bold italic text-orange-400">Disqualified</span>
//                                                     ) : (
//                                                         <>
//                                                             <span className={`font-mono font-bold text-lg md:text-xl ${isWin ? 'text-green-400' : 'text-[var(--text-primary)]'}`}>
//                                                                 {myData.score}
                                    ...
                                    return (
                                        <div key={idx} className={`bg-[var(--bg-secondary)] p-4 rounded-xl border flex items-center justify-between hover:border-accent transition-all shadow-sm group ${isDisqualified ? 'border-orange-500/30' : 'border-[var(--border-color)]'}`}>
                                            ...
                                            <div className="text-right pl-4 flex flex-col items-end justify-center">
                                                ...
                                                {/* ✅ ENHANCED ELO DISPLAY: Supports the "Slightly Increased on Loss" logic */}
                                                {eloChange !== 0 && (
                                                    <div className={`text-xs font-bold flex items-center gap-1 ${eloChange > 0 ? 'text-green-500' : 'text-red-500'}`}>
                                                        {eloChange > 0 ? <TrendingUp size={12} /> : <TrendingDown size={12} />}
                                                        {eloChange > 0 ? '+' : ''}{eloChange} ELO
                                                    ...
                                                    <div className="text-[10px] text-gray-500 font-bold uppercase tracking-tighter">No Rating Change</div>
                                                )}
                                            </div>
                                        </div>
                                    );
