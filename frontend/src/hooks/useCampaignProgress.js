...
const normalizeCompletedMap = (progressData) => {
  if (!progressData) return MOCK_PROGRESS;

  if (Array.isArray(progressData.completedNodes)) {
    return progressData.completedNodes.reduce((acc, entry) => {
      if (!entry?.nodeId) return acc;
      acc[entry.nodeId] = {
        stars: entry.starsAwarded ?? entry.stars ?? 0,
        completedAt: entry.completedAt ?? null,
...
// ── Main hook ─────────────────────────────────────────────────────────────────
export const useCampaignProgress = (externalProgress = null) => {
  // 1. STATEFUL FIX: Manage progress internally to allow instant UI updates
  const [currentProgress, setCurrentProgress] = useState(externalProgress);

  // Sync internal state if the parent component fetches fresh data
  useEffect(() => {
    setCurrentProgress(externalProgress);
  }, [externalProgress]);

  const completedMap = useMemo(
    () => normalizeCompletedMap(currentProgress),
    [currentProgress]
  );

  const unlockedSet = useMemo(() => {
    if (Array.isArray(currentProgress?.unlockedNodes)) {
      return new Set(currentProgress.unlockedNodes);
    }
    return null;
  }, [currentProgress]);
  ...
  const enrichedZones = useMemo(() => {
    return ZONES.map((zone, zIdx) => {
      ...
      const isZoneLocked = nodes.state === 'locked';
      const completedCount = nodes.filter(n => n.state === 'completed').length;
      const progressPct    = Math.round((completedCount / nodes.length) * 100);
  ...
  // ── UPDATE HANDLERS (Call these from your components) ───────────────────────

  // Call this if your API returns the FULL updated progress object after a win
  const setProgressData = useCallback((newData) => {
    setCurrentProgress(newData);
  }, []);

  // Call this if your API only returns "Success", to update the UI instantly
  const markNodeCompletedLocally = useCallback((nodeId, stars = 3) => {
    setCurrentProgress(prev => {
      // Handle professional Backend shape
      if (prev && Array.isArray(prev.completedNodes)) {
        return {
          ...prev,
          completedNodes: [...prev.completedNodes, { nodeId, stars, completedAt: new Date().toISOString() }],
          // CRITICAL FIX: Wiping unlockedNodes forces the hook to fall back to 
          // `deriveNodeState`, instantly calculating and unlocking the next node locally!
          unlockedNodes: null 
        };
      }
      ...
      return {
        ...prev,
        [nodeId]: { stars, completedAt: new Date().toISOString() }
      };
    });
  }, []);
